...
buildFile portData defsData =
    let
        ...
        usedIn haystack needle =
            List.any (String.contains needle) haystack

        portOut =
            emitIf (usedIn portData "PortOut<")
                """
interface PortOut<T> {
...
"""
        ...
        portResult =
            emitIf
                (usedIn
                    (portData ++ List.map .body defsData)
                    "PortResult<"
                )
                """
...
$PORT_OUT$PORT_IN$DEFS$PORT_RESULT"""
        |> String.replace "$PORTS" ports
        |> String.replace "$PORT_OUT" portOut
        |> String.replace "$PORT_IN" portIn
        |> String.replace "$DEFS" defs
        |> String.replace "$PORT_RESULT" portResult
